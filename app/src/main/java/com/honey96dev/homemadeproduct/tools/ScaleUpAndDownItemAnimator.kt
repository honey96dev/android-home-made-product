...
class ScaleUpAndDownItemAnimator : DefaultItemAnimator() {
    ...
    override fun animateChange(oldHolder: RecyclerView.ViewHolder, newHolder: RecyclerView.ViewHolder, preInfo: RecyclerView.ItemAnimator.ItemHolderInfo, postInfo: RecyclerView.ItemAnimator.ItemHolderInfo): Boolean {
        ...
        if (runningInfo != null) {
            ...
            prevAnimPlayTime = if (firstHalf) runningInfo.zoomInAnimator!!.currentPlayTime else runningInfo.zoomOutAnimator.currentPlayTime
        ...
        val runningAnimInfo = AnimatorInfo(overallAnimation, scaleUpAnimator, scaleDownAnimator)
        animatorMap[newHolder] = runningAnimInfo
    ...
    override fun endAnimation(item: RecyclerView.ViewHolder) {
        ...
        if (!animatorMap.isEmpty) {
            val numRunning = animatorMap.size
    ...
    private class AnimatorInfo internal constructor(internal val overallAnim: Animator, internal val zoomInAnimator: ValueAnimator?, internal val zoomOutAnimator: ValueAnimator)

    companion object {
        private val MAX_SCALE = 3f
        private val DURATION_SCALE: Long = 300
    }
}